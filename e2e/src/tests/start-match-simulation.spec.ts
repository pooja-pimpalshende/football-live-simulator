...
test('should show start button and initial scores', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="total-goals"]')).toBeHidden();
});
...
test('should start simulation and increment goals', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="total-goals"]')).toHaveText('Total Goals: 2', {
    timeout: 25000,
  });
});

test('should finish simulation after 90s and show restart button', async ({ page }) => {
  await page.goto('/');
  await page.click('[data-testid="start-button"]');
...
test('should not increment goals after simulation ends', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="total-goals"]')).toHaveText('Total Goals: 9', {
    timeout: 95000,
  });
...
test('Should reset and start simulation again after restart', async ({ page }) => {
  await page.goto('/');
  await page.click('[data-testid="start-button"]');
